    ...
    switch (value[0]) {
      case ":":
        ...
        if (this.cleanedTargetName === "innerHTML") {
          ...
          this.binding = (s4, c4) => DOM.createHTML(binding(s4, c4));
        }
        break;
      case "?":
        this.cleanedTargetName = value.substr(1);
...
function compileAttributes(context, node, includeBasicValues = false) {
  ...
  for (let i4 = 0, ii = attributes2.length; i4 < ii; ++i4) {
    const attr2 = attributes2[i4];
    ...
    for (let i4 = 0, ii = attributeLists.length; i4 < ii; ++i4) {
      ...
      for (let j2 = 0, jj = list.length; j2 < jj; ++j2) {
        ...
        if (typeof config === "string") {
          attributes2.push(new _AttributeDefinition(Owner, config));
        } else {
          attributes2.push(new _AttributeDefinition(Owner, config.property, config.attribute, config.mode, config.converter));
        }
      }
    }
    return attributes2;
...
var FASTElementDefinition = class {
  ...
  constructor(type, nameOrConfig = type.definition) {
    ...
    const attributes2 = AttributeDefinition.collect(type, nameOrConfig.attributes);
    const observedAttributes = new Array(attributes2.length);
    const propertyLookup = {};
    const attributeLookup = {};
    for (let i4 = 0, ii = attributes2.length; i4 < ii; ++i4) {
      const current = attributes2[i4];
  ...
  define(registry = customElements) {
    ...
    if (fastRegistry.register(this)) {
      const attributes2 = this.attributes;
      const proto = type.prototype;
      for (let i4 = 0, ii = attributes2.length; i4 < ii; ++i4) {
        Observable.defineProperty(proto, attributes2[i4]);
      }
...
var ContainerImpl = class _ContainerImpl {
  constructor(owner, config) {
    ...
    if (owner instanceof Node) {
      owner.addEventListener(DILocateParentEventType, (e6) => {
        if (e6.composedPath()[0] !== this.owner) {
          e6.detail.container = this;
...
function cacheCallbackResult(fun) {
  return function(handler, requestor, resolver) {
    ...
    const t3 = fun(handler, requestor, resolver);
    cache.set(resolver, t3);
    return t3;
  };
}
...
// libs/vscode/nx-cloud-onboarding-webview/src/list-entry.ts
var ListEntry = class extends s3 {
  constructor() {
    super(...arguments);
  ...
  render() {
    return T`
      ...
      >
        ...
        ></span>
        <span
          style="${this.completed ? "text-decoration: line-through; color: var(--vscode-disabledForeground);" : ""}; line-height: 1.2rem; display: inline-flex; align-items: center;"
        >
          ${this.text}
        </span>
      </div>
...
__decorateClass([
  n5()
], ListEntry.prototype, "completed", 2);
__decorateClass([
  n5()
], ListEntry.prototype, "text", 2);
ListEntry = __decorateClass([
  e3("list-entry")
], ListEntry);
...
var Root = class extends s3 {
  ...
  render() {
    ...
    return T`
      <div style="font-size: var(--vscode-font-size)">
        ${completed.map(
      (text) => T`<list-entry .text=${text} .completed=${true}></list-entry>`
    )}
        ${notCompleted.map(
      (text) => T`<list-entry .text=${text} .completed=${false}></list-entry>`
    )}
      </div>
  ...
  getElements() {
    ...
    (this.cloudOnboardingInfo?.hasAffectedCommandsInCI ? completed : notCompleted).push("Use affected commands in your CI configuration");
    (this.cloudOnboardingInfo?.isConnectedToCloud ? completed : notCompleted).push("Enable remote caching with Nx Cloud");
    (this.cloudOnboardingInfo?.isConnectedToCloud ? completed : notCompleted).push("Enable task distribution with Nx Cloud");
    (this.cloudOnboardingInfo?.isConnectedToCloud && this.cloudOnboardingInfo?.isWorkspaceClaimed ? completed : notCompleted).push("Finish connecting your repository to Nx Cloud");
    return {
      completed,
      notCompleted
    };
...
__decorateClass([
  n5({ type: Object })
], Root.prototype, "cloudOnboardingInfo", 2);
